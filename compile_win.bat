...
REM Check if cl.exe is already available (GitHub Actions / CI case)
where cl >nul 2>&1
if not errorlevel 1 (
    echo [OK] cl.exe found in PATH (CI environment)
    goto :build_env_ready
)

REM If not in CI, do normal Visual Studio detection
...
if exist "!VSINSTALLDIR!\VC\Auxiliary\Build\vcvarsall.bat" (
    ...
    if not errorlevel 1 (
        echo [OK] Visual Studio environment set up
        goto :build_env_ready
    )
)
